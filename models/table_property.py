...
class TableProperty:
    ...
    is_incrementing: bool  # オートインクリメントかどうか
    is_timestamps: bool  # Laravelで使用する「timestamps」の設定
    foreign_keys: list[ForeignKey]  # 紐づいているキーの設定情報
    column_list: List[ColumProperty]  # カラム情報の配列(ColumProperty)
    is_soft_deleted: bool  # ソフトデリート機能のテーブルになるかどうか
